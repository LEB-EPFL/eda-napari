...
class Frame_rate_Widget(QWidget):
   ...
   def plot_times(self):
      ...
      self.line_1=self.ax.axvline(0,0,1,linewidth=1, color='indianred')#initilaise a vertical line
   ...
   def plot_frame_rate(self,unit_frame_r='Hz'):
      ...
      self.line_2=self.ax2.axvline(0,0,1,linewidth=1, color='indianred')#initialise a vertical line
   ...
   def plot_slider_position(self,event): #event information stored in "event"
      ...
      self.line_2.set_xdata(current_frame) #update line
      self.fig.canvas.draw()
      self.fig2.canvas.draw()
